...
class Admin {
  ...
  /* ================= DELETE ADMIN ================= */
static delete(admin_id) {
    const sql = `DELETE FROM mukta_admin_db WHERE admin_id = ?`;
...
static findById(admin_id) {
    const sql = `SELECT * FROM mukta_admin_db WHERE admin_id = ? LIMIT 1`;
    return new Promise((resolve, reject) => {
...
/* ================= UPDATE STATIC OTP ================= */
static updatePassOtp(admin_id, pass_otp) {
    const sql = `
        UPDATE mukta_admin_db
        SET pass_otp = ?
        WHERE admin_id = ?
    ...
    return new Promise((resolve, reject) => {
        db.query(sql, [pass_otp, admin_id], (err, result) => {
            if (err) return reject(err);
            resolve(result);
        });
    });
